...
<head>
    ...
    <title>NexaTrade | Kayıt Ol</title>
    ...
    <style>
        ...
        /* ---- Sag/alt taraf: kayit formu karti ---- */
        .glass-panel {
        ...
        /* ---- Hatali kayit: panel hafifce sallanarak dikkat ceker (bkz. asagida inline style) ---- */
        @keyframes shake {
        ...
        /* ---- Kayıt butonu: gonderim sirasinda donen bir yukleme ikonu gosterir ---- */
        ...
        <div class="hero-panel lg:w-1/2 xl:w-3/5 flex flex-col justify-center px-8 sm:px-12 lg:px-20 py-8 sm:py-10 lg:py-0 text-white">
            ...
            <div class="relative stagger max-w-md mx-auto lg:mx-0 w-full">
                ...
                <p class="hidden lg:block text-sm text-gray-400 mb-8 max-w-sm">
                    Saniyeler içinde hesabını oluştur; piyasayı 7/24 tarayan, GPT destekli karar
                    motoruyla çalışan otonom ticaret terminaline katıl.
                </p>
            ...
            <div class="w-full max-w-sm relative glass-panel rounded-2xl p-8" style="<?= !empty($error) ? 'animation: panel-in 0.5s cubic-bezier(0.16, 1, 0.3, 1) both, shake 0.5s cubic-bezier(0.36, 0.07, 0.19, 0.97) 0.5s both;' : '' ?>">
                <div class="stagger">
                    <div class="mb-6 lg:text-left text-center">
                        <h2 class="font-display text-xl font-bold text-gray-900">Hesap oluştur</h2>
                        <p class="text-xs text-gray-500 mt-1">Birkaç saniyede kayıt ol, terminale başla.</p>
                    </div>
                    ...
                    <form method="POST" action="<?= htmlspecialchars(Url::to('/register'), ENT_QUOTES, 'UTF-8') ?>" class="space-y-4" onsubmit="var b=document.getElementById('registerBtn'); b.classList.add('btn-loading'); b.disabled=true;">
                        <div>
                            <label for="name" class="block text-[10px] tracking-widest text-gray-500 mb-1">AD SOYAD</label>
                            <input type="text" id="name" name="name" required autofocus class="w-full bg-gray-100 border border-black/10 rounded-lg px-3 py-2 text-sm text-gray-800 font-mono-tech focus:outline-none focus:border-violet-400/60 focus:ring-1 focus:ring-violet-400/40">
                        </div>
                        <div>
                            <label for="email" class="block text-[10px] tracking-widest text-gray-500 mb-1">E-POSTA</label>
                            <input type="email" id="email" name="email" required class="w-full bg-gray-100 border border-black/10 rounded-lg px-3 py-2 text-sm text-gray-800 font-mono-tech focus:outline-none focus:border-violet-400/60 focus:ring-1 focus:ring-violet-400/40">
                        </div>
                        <div>
                            <label for="password" class="block text-[10px] tracking-widest text-gray-500 mb-1">ŞİFRE (EN AZ 8 KARAKTER)</label>
                            <input type="password" id="password" name="password" minlength="8" required class="w-full bg-gray-100 border border-black/10 rounded-lg px-3 py-2 text-sm text-gray-800 font-mono-tech focus:outline-none focus:border-violet-400/60 focus:ring-1 focus:ring-violet-400/40">
                        </div>
                        <div>
                            <label for="password_confirm" class="block text-[10px] tracking-widest text-gray-500 mb-1">ŞİFRE (TEKRAR)</label>
                            <input type="password" id="password_confirm" name="password_confirm" minlength="8" required class="w-full bg-gray-100 border border-black/10 rounded-lg px-3 py-2 text-sm text-gray-800 font-mono-tech focus:outline-none focus:border-violet-400/60 focus:ring-1 focus:ring-violet-400/40">
                        </div>
                        <button type="submit" id="registerBtn" class="w-full flex items-center justify-center gap-2 rounded-lg bg-violet-500 hover:bg-violet-400 transition-colors text-white text-sm font-semibold py-2.5">
                            <span class="btn-spinner"></span>
                            <span class="btn-label">Kayıt Ol</span>
                        </button>
                    </form>

                    <p class="text-center text-xs text-gray-500 mt-6">
                        Zaten hesabın var mı?
                        <a href="<?= htmlspecialchars(Url::to('/login'), ENT_QUOTES, 'UTF-8') ?>" class="text-violet-600 hover:text-violet-500 transition-colors">Giriş yap</a>
                    </p>
                </div>
            </div>
        </div>
